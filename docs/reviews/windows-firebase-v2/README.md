...
이 문서는 2026-09-22 초기 구현 검증 기록이다. 아래 rollout 상태, 설치 파일 경로와 검증 결과는
그 시점의 증거이며 현재 운영 상태를 나타내지 않는다. 현재 Windows 클라이언트의 선택자 동작은
[FirebaseRealtimeRolloutSelector.cs](../../../windows/src/Sidey.Infrastructure/Realtime/FirebaseRealtimeRolloutSelector.cs)를 참조한다.
...
따라서 이 binary는 Firebase v2 기능이 통합된 client지만, 검증 당시 production 서버가 OFF를 반환했으므로 실제 사용자
세션은 legacy Supabase를 사용했다. 서버가 ON을 반환하는 세션에서는 설치 파일 교체 없이
Firebase chat/hint plane이 활성화된다. permission denial을 이유로 자동 legacy downgrade하지 않는다.

## 남은 원격 검증

- 당시 rollout OFF 상태에서는 인증된 production Firebase SSE/callable 성공 경로를 실행할 수 없었다.
- selector ON에서 두 SSE 연결, token rotation overlap, old↔new mixed-client smoke를 검증했는지는 별도 운영 증거로 확인해야 한다.
...
이 항목들은 당시 installer 생성이나 구형 client 호환을 막지 않았다. 운영 검증의 완료 여부는 별도로 확인해야 한다.
