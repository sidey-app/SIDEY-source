...
App Store client의 지원 환경은 Apple Silicon Mac의 macOS 26 이상이다. 최소 OS와
CPU architecture의 실행 가능한 계약은
[`Xcode project`](../../macos/SIDEY.xcodeproj/project.pbxproj)가 소유한다. 실제 설치
가능 여부는 App Store의 제공 상태에 따른다.
