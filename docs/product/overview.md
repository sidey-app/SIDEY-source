...
최초 온보딩 동안에는 Dock에 app으로 표시한다. 온보딩을 완료하면 로그인 항목을 통한
자동 실행과 사용자의 수동 실행 모두 메뉴바 accessory app으로 동작한다. 설정 또는 최근
기록 창을 열어도 Dock icon을 다시 표시하지 않는다. 메뉴바 메뉴에서 설정, 최근 기록과
종료에 접근한다.
