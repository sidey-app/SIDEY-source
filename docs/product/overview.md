...
최근 기록 창의 항상 위 버튼으로 다른 창 위에 유지할 수 있다. 표시된 최근 기록
창에서는 `Alt+위쪽 화살표`로 항상 위를 켜고 `Alt+아래쪽 화살표`로 끈다. 설정은 각 조합을
...
Windows에서 캐릭터를 한 번 클릭하면 100ms 뒤 입력창을 애니메이션과 함께 토글한다. 그 안에 두 번
