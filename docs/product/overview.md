...
`Ctrl+Alt+R`로 최근 기록 열기다. 설정에서 항목을 선택한 뒤 수정 키와 일반 키를
함께 누르면 전체 키 조합을 기능별로 바꿀 수 있다. 수정 키만 누르거나 수정 키 없는
일반 키는 등록하지 않으며, `Escape`로 입력을 취소한다. 이미 사용 중인 조합을
입력하면 두 기능의 단축키를 서로 바꾼다.
변경은 즉시 적용해 로컬에 저장한다. 다른 앱과 등록이 충돌하면 해당 기능만 사용할 수
없고 나머지 단축키는 유지하며, 트레이 알림으로 안내한 뒤 다음 실행에서 다시 등록한다.
키를 누른 채 유지해도 반복 실행하지 않는다.
