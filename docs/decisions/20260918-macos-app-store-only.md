...
- Refined by: [macOS Direct 공개 배포물 삭제](20260921-remove-macos-direct-releases.md)
...
줄이고 App Store판의 안정성과 실시간 이관 작업에 집중한다. 이 결정 당시에는 과거 공개
release 기록도 보존했지만, 해당 보존 범위는 후속
[macOS Direct 공개 배포물 삭제](20260921-remove-macos-direct-releases.md) 결정으로 바뀌었다.
기존 사용자 계정, 메시지, 구매 원본은 삭제하지 않는다. 기존 distribution 사이의
계정·저장소 자동 병합 금지 경계는 유지하며, 배포 종료가 기존 고객의 entitlement 회수나
자동 데이터 이전을 의미하지 않는다.
